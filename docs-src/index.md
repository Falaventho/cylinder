...
Cylinder keeps framework-level configuration intentionally small. The built-in options on `Cylinder()` are:
...
`abort_extra` is an optional argument to `Cylinder()` that lets you register additional HTTP exception
...
Cylinder’s built-in logging can be customized through `Cylinder()` and through the log formatter.
...
`request_id_header` is an optional argument to `Cylinder()` that specifies which incoming HTTP header
should be used as the request ID.
